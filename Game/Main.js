...
(function (Doom) {
    ...
    ƒ.AudioManager.default.listenWith(Doom.avatar.getComponent(ƒ.ComponentAudioListener));
    async function hndLoad(_event) {
        ...
        floor.mtxLocal.scale(ƒ.Vector3.ONE(Doom.sizeWall * Doom.numWalls));
        floor.getComponent(ƒ.ComponentMaterial).pivot.scale(ƒ.Vector2.ONE(Doom.numWalls));
        ...
        Doom.avatar.addComponent(cmpCamera);
        Doom.avatar.addComponent(new ƒ.ComponentTransform());
        Doom.avatar.mtxLocal.translate(ƒ.Vector3.Z(10));
        Doom.avatar.mtxLocal.rotate(ƒ.Vector3.Y(180));
        root.appendChild(Doom.avatar);
        Doom.avatar.addComponent(new ƒ.ComponentAudioListener);
        Doom.viewport = new ƒ.Viewport();
        Doom.viewport.initialize("Viewport", root, cmpCamera, canvas);
        Doom.viewport.draw();
    ...
    function hndLoop(_event) {
        ...
        for (let enemy of enemies.getChildren()) {
            enemy.update();
            ƒ.AudioManager.default.listenTo(enemy);
        }
    ...
    function moveAvatar(_speed, _rotation, _strafe) {
        Doom.avatar.mtxLocal.rotateY(_rotation);
        let posOld = Doom.avatar.mtxLocal.translation;
        Doom.avatar.mtxLocal.translateZ(_speed);
        Doom.avatar.mtxLocal.translateX(_strafe);
        ...
        if (bouncedOff.length == 0)
            return;
        console.log("Stuck!");
        Doom.avatar.mtxLocal.translation = posOld;
    }
    function bounceOffWalls(_walls) {
        let bouncedOff = [];
        let posAvatar = Doom.avatar.mtxLocal.translation;
        for (let wall of _walls) {
            let posBounce = wall.calculateBounce(posAvatar, 1);
            if (posBounce) {
                Doom.avatar.mtxLocal.translation = posBounce;
    ...
    async function createEnemies() {
        ...
        Doom.Enemy.generateSprites(coatSprite);
        for (let i = 0; i < 1; i++)
            enemies.appendChild(new Doom.Enemy("Cacodemon" + i, ƒ.Vector3.Z(3)));
    ...
    function createWalls() {
        ...
        walls.appendChild(new Doom.Wall(ƒ.Vector2.ONE(3), ƒ.Vector3.Y(Doom.sizeWall / 2), ƒ.Vector3.ZERO(), mtrWall));
        walls.appendChild(new Doom.Wall(ƒ.Vector2.ONE(3), ƒ.Vector3.SCALE(new ƒ.Vector3(0.5, 1, -0.866), Doom.sizeWall / 2), ƒ.Vector3.Y(120), mtrWall));
        walls.appendChild(new Doom.Wall(ƒ.Vector2.ONE(3), ƒ.Vector3.SCALE(new ƒ.Vector3(-0.5, 1, -0.866), Doom.sizeWall / 2), ƒ.Vector3.Y(-120), mtrWall));
        for (let i = -Doom.numWalls / 2 + 0.5; i < Doom.numWalls / 2; i++) {
            walls.appendChild(new Doom.Wall(ƒ.Vector2.ONE(3), ƒ.Vector3.SCALE(new ƒ.Vector3(-Doom.numWalls / 2, 0.5, i), Doom.sizeWall), ƒ.Vector3.Y(90), mtrWall));
            // for (let i: number = -numWalls / 2 + 0.5; i < numWalls / 2; i++)
            walls.appendChild(new Doom.Wall(ƒ.Vector2.ONE(3), ƒ.Vector3.SCALE(new ƒ.Vector3(Doom.numWalls / 2, 0.5, i), Doom.sizeWall), ƒ.Vector3.Y(-90), mtrWall));
            // for (let i: number = -numWalls / 2 + 0.5; i < numWalls / 2; i++)
            walls.appendChild(new Doom.Wall(ƒ.Vector2.ONE(3), ƒ.Vector3.SCALE(new ƒ.Vector3(i, 0.5, -Doom.numWalls / 2), Doom.sizeWall), ƒ.Vector3.Y(0), mtrWall));
            // for (let i: number = -numWalls / 2 + 0.5; i < numWalls / 2; i++)
            walls.appendChild(new Doom.Wall(ƒ.Vector2.ONE(3), ƒ.Vector3.SCALE(new ƒ.Vector3(i, 0.5, Doom.numWalls / 2), Doom.sizeWall), ƒ.Vector3.Y(180), mtrWall));
        }
        return walls;
    }
})(Doom || (Doom = {}));
